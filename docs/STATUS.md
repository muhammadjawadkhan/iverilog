...
| Parameterized classes | **Partial**: ANSI `class C #(type T = int, ...);` with defaults, plus MVP explicit `C#(byte)` specialization (lazy clone + override). See [`param-classes.md`](param-classes.md) / [`examples/param_classes`](../examples/param_classes). |
